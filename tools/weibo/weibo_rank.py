...
def get_text_net(flag,weibo_text,word):
    ...
    for i in range(0,len(weibo_text)):
        ...
        for w in word:
            k1,k2 = w[1].split('_')
            c[str(k)] = c[str(k)] + str(weibo_text[i][0]).count(str(k1))*w[0] + str(weibo_text[i][0]).count(str(k2))*w[0]
            if w not in w_list:
                w_list.append(str(w))
    ...
    with open('./text%s.csv' % flag, 'wb') as f:
        ...
        for i in range(0,len(data)):
            k = data[i][1]
            writer.writerow((news_data[k][0],news_data[k][1],news_data[k][2],news_data[k][3],news_data[k][4],news_data[k][5],news_data[k][6]))
...
def keyword(flag):
    ...
    for mid,t,publish,a,b,c,d in reader:#按标题归类
        data.append([mid,t,publish,a,b,c,d])
...
if __name__ == '__main__':
    keyword('enemy_20141115_20141121')
##    keyword('domain_20141115_20141121')
##    keyword('corp_20141115_20141121')
